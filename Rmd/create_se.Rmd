...
# ロガー設定の取得
logger_settings <- setup_logger("create_se", params$experiment_id, "TRACE")
# 設定の適用
...
# ログ開始メッセージ
futile.logger::flog.info("--- create_se のロガー設定完了 (PID: %d) ---", Sys.getpid())
futile.logger::flog.info("モジュールログファイルパス: %s", logger_settings$module_log_path)
futile.logger::flog.info("ログレベル: %s", logger_settings$threshold)

# 入力パラメータのログ記録
flog.info("--- create_se.Rmd の実行開始 ---")
flog.info("実験ID: %s", params$experiment_id)
flog.info("入力パラメータ:")
flog.trace("- experiment_id: %s", params$experiment_id)
flog.trace("- counts_file_path: %s", params$counts_file_path)
flog.trace("- metadata_file_path: %s", params$metadata_file_path)
flog.trace("- gene_id_column: %s", params$gene_id_column)
flog.trace("- sample_id_column: %s", params$sample_id_column)
flog.trace("- biomart_host: %s", params$biomart_host)
flog.trace("- biomart_dataset: %s", params$biomart_dataset)
flog.trace("- biomart_attributes: %s", paste(params$biomart_attributes, collapse=", "))
```
...
if(any(is.na(sample_metadata_ordered[[params$sample_id_column]]))) {
    ...
    msg <- sprintf("メタデータの並べ替え中に問題が発生しました。カウントデータにあるがメタデータにないサンプルIDが存在する可能性があります。NA行インデックス: %s", paste(na_rows, collapse=", "))
    flog.error(msg)
    # ここで停止させるか、警告に留めるかは要件による。安全のため停止。
    stop(msg)
}
...
count_matrix <- as.matrix(counts_data[, setdiff(colnames(counts_data), params$gene_id_column)])
...
plot_protein_coding_library_size <- function(se) {
  ...
  protein_coding_counts <- assay(se, "counts")[is_protein_coding, ]
